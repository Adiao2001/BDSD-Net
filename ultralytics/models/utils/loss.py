...
class DETRLoss(nn.Module):
    ...
    def _get_loss_bbox(self, pred_bboxes, gt_bboxes, postfix=""):
        ...
        try:
            ...
            # 一类IoU损失函数
            if useloss == 'CIoU':
                loss[name_giou] = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, CIoU=True) # 🎈 对应IoU损失函数注释即可运行代码
            elif useloss == 'MPDIoU':
                loss[name_giou] = 1.0 - bbox_iou(pred_bboxes, gt_bboxes, xywh=True, MPDIoU=True)
                ...
                from ultralytics.utils.NewLoss.iouloss import bbox_multi_iou, bbox_focal_multi_iou
                loss[name_giou] = 1.0 - bbox_multi_iou(pred_bboxes, gt_bboxes, xywh=True, SIoU=True)
            elif useloss == 'DIoU':
