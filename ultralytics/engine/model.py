...
class Model(nn.Module):
    ...
    def _load_teacher(self, weights: str, task=None):
        """
        Initializes a new model and infers the task type from the model head.
        ...
        suffix = Path(weights).suffix
        if suffix == ".pt":
            self.model_teacher, self.ckpt_teacher = attempt_load_one_weight(weights)
    ...
    def train(self, trainer=None, **kwargs):
        ...
        if not args.get("resume"):  # manually set model only if not resuming
            ...
            if self.model_teacher is not None:
                self.trainer.model_teacher = self.trainer.get_model(weights=self.model_teacher if self.ckpt_teacher else None, cfg=self.model.yaml)
